...
import type { CalendarDay, DateRange, WeekNumberProps } from "react-day-picker";
...
export function DateRangePicker({ date, setDate }: DateRangePickerProps) {
  ...
  const CustomWeekNumber = ({ week, ...props }: WeekNumberProps) => {
    const handleClick = () => {
      const minDate = new Date(2025, 0, 1);

      // Get all dates from the week
      const allDates = week.days.map((day: CalendarDay) => day.date);
    ...
    return (
      <td
        ...
        className={cn(
          "text-muted-foreground rdp-week_number text-[0.8rem] select-none",
          hasEnabledDates
